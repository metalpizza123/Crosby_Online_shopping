...
<?php
	...
	if ($result -> num_rows>0) {
		while ($rows = $result -> fetch_assoc()) {
			...
			$queryname = "SELECT name from team where teamid = '$foundhomeID'";
			$nameresult = $mysqli -> query($queryname);
			if ($nameresult -> num_rows>0) {
				while ($namerows = $nameresult -> fetch_assoc()) {
					$foundhomename = $namerows['name'];
				}
			}
			//Look for away name from ID
			$queryname = "SELECT name from team where teamid ='$foundawayID'";
			$nameresult = $mysqli -> query($queryname);
			if ($nameresult -> num_rows >0) {
				while ($namerows = $nameresult -> fetch_assoc()) {
					$foundawayname = $namerows['name'];
				}
			}
			if ($played == 0) {
